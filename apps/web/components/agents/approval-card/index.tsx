...
function getStatusBadgeClass(status: ApprovalCardStatus) {
  if (status === "pending" || status === "changes-requested") {
    return "border-amber-500/30 bg-amber-50 text-amber-600";
  }
  if (status === "submitting") {
    return "border-blue-500/30 bg-blue-50 text-blue-600";
  }
  if (status === "approved" || status === "answered") {
    return "border-emerald-500/30 bg-emerald-50 text-emerald-600";
  }
  return "border-rose-500/30 bg-rose-50 text-rose-600";
}
...
export function ApprovalCard({
  ...
  return (
    <div
      ...
      aria-busy={busy}
      className={cn(
        "w-full overflow-hidden rounded-2xl bg-white border border-gray-200 p-4 text-sm",
        className,
      )}
    >
      <div className="flex items-start gap-3">
        <span
          aria-hidden="true"
          className={cn(
            "grid size-5 shrink-0 place-items-center text-gray-400",
        ...
        <div className="min-w-0 flex-1">
          <div className="flex min-w-0 items-start gap-3">
            <h3 className="min-w-0 flex-1 text-base font-medium leading-5 text-black">
            ...
            {onDismiss ? (
              <button
                ...
                aria-label="Dismiss"
                onClick={onDismiss}
                className="grid size-5 shrink-0 place-items-center rounded-full text-gray-400 outline-none transition-colors hover:text-black focus-visible:ring-2 focus-visible:ring-gray-300"
              >
                <X className="size-4" />
              </button>
            ) : null}
